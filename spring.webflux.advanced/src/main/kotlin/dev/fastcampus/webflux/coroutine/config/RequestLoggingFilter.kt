...
class RequestLoggingFilter: WebFilter {
    override fun filter(exchange: ServerWebExchange, chain: WebFilterChain): Mono<Void> {
        ...
        logger.info { "url: [${request.method}] ${request.path}, ip: ${request.remoteAddress}" }
            ...
            override fun getBody(): Flux<DataBuffer> {
                return super.getBody().doOnNext{ buffer ->
                    ByteArrayOutputStream().use { output ->
                        Channels.newChannel(output).use {
                            it.write(buffer.readableByteBuffers().next())
                        }
        ...
        return chain.filter(exchange.mutate().request(loggingRequest).build())
    }
}